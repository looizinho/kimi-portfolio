...
export function WorkCard({ work }: WorkCardProps) {
  ...
  return (
    <article className="group relative overflow-hidden rounded-3xl border border-border bg-card/50 dark:border-white/10 dark:bg-white/5 backdrop-blur-sm">
      {work.image && (
              ...
              <DialogTrigger asChild>
                <button
                  ...
                  className={cn(
                    ...
                    'hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70'
                  )}
                  aria-label={`Reproduzir vídeo de ${work.title}`}
                >
                  <span className="flex items-center justify-center rounded-full border border-border bg-card/60 dark:border-white/40 dark:bg-black/40 p-4 shadow-lg backdrop-blur">
                    <Play className="h-7 w-7 translate-x-[1px]" />
                  </span>
                </button>
              </DialogTrigger>
              <DialogContent
                className="border-border bg-background/95 dark:border-white/10 dark:bg-zinc-950/95 p-0 sm:max-w-4xl"
              >
            ...
            {work.tags.map((tag, index) => (
              <span
                ...
                className={cn(
                  ...
                  'bg-card/60 text-foreground/90 border border-border dark:bg-white/10 dark:text-white/90 dark:border-white/10'
                )}
              >
                {tag}
              </span>
